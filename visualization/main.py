...
def generate_map():
    ...
    # Execute a query to retrieve data from the table (use DISTINCT ON to get one row per CML)
    cur.execute(
...
def generate_time_series_plot(cml_id=None):
    ...
    plot = alt.Chart(df).mark_line().encode(x="time:T", y="rsl:Q")
